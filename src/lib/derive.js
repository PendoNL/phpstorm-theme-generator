...
export function deriveTokens(palette,variant){
  const {seeds}=transformSeeds(palette,variant);
  const [C1,C2,C3,C4,C5]=seeds;
  ...
  const bgCap = neon?0.06:tinted?0.035:paper?0.014:0.012, bgMin = neon?0.03:tinted?0.025:paper?0.010:0.008;
  ...
  T.synConstant = fix('synConstant',fromOklch(o4.l,o4.c,hConst), T.bgEditor, synFloor);
  T.synOperator = T.fgMuted;
  T.synPunct    = T.fgSubtle;
  T.synMetadata = desat(T.synType,0.8);
  T.synInvalid  = T.semError;
